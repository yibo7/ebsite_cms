 ...
 * 加载弹出框
 * @param msg
 * @param time 多长时间关闭，时间为秒，可不传（手动关闭）
 */
function blockTips(msg, time) {
    ...
    var msgHtml = '<div class="blockTips"><img width="30" src="/loading.gif"> ' + msg + '</div>';
    iTime = iTime * 1000;
    In.ready('blockui', function () {
        //执行代码
        $.blockUI({
...
function fav_content(id){
    const prams = {"data_id":id};
    post_form("/api/fav_content",prams,function(rz){
        const btn = document.getElementById('favorit-btn');
        if (rz.code === 0) {
            // 收藏成功
            toast(rz.data || '成功加入收藏夹!');
            if (btn) {
                btn.innerHTML = '★ 已收藏';
                btn.dataset.faved = 'true';
                btn.classList.add('is-faved');
            }
        } else if (rz.msg === '已取消收藏!') {
            // 取消收藏
            toast(rz.msg);
            if (btn) {
                btn.innerHTML = '☆ 收藏';
...
function check_fav_status(id){
    if (!id) return;
    get_json('/api/check_fav?data_id=' + id, function(rz){
        if (rz.code === 0 && rz.data && rz.data.favorited) {
            const btn = document.getElementById('favorit-btn');
            if (btn) {
                btn.innerHTML = '★ 已收藏';
                btn.dataset.faved = 'true';
